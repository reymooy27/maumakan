...
async function main() {
  ...
  const kupangPlaces = [
    ...
    },
    {
      ...
      menu: [
        { name: 'Sei Babi Reguler', price: 40000, description: 'Daging asap khas Kupang with sambal lu\'at' },
  ...
  // Performance Test: Generate 500 more randomized places around Kupang
  console.log('Generating 500 randomized places for performance testing...');
  const placeTypes = [PlaceType.restaurant, PlaceType.cafe, PlaceType.food_stall];
  const placeNames = [
    'Warung', 'Cafe', 'Resto', 'Rumah Makan', 'Kedai', 'Bakso', 'Mie Ayam', 'Sei', 'Kopi', 'Dapur'
  ];
  const suffixes = [
    'Sejahtera', 'Makmur', 'Sederhana', 'Nikmat', 'Mantap', 'Kupang', 'Timor', 'Oebobo', 'Fatu Feto', 'El Tari'
  ];

  const randomPlaces = Array.from({ length: 500 }).map((_, i) => {
    const type = placeTypes[Math.floor(Math.random() * placeTypes.length)];
    const name = `${placeNames[Math.floor(Math.random() * placeNames.length)]} ${suffixes[Math.floor(Math.random() * suffixes.length)]} ${i + 1}`;
    
    return {
      name,
      type,
      lat: -10.14 - Math.random() * 0.06, 
      lng: 123.58 + Math.random() * 0.07,
      rating: 3 + Math.random() * 2,
      avgPrice: 10000 + Math.floor(Math.random() * 90) * 1000,
      openTime: 480 + Math.floor(Math.random() * 120),
      closeTime: 1200 + Math.floor(Math.random() * 240),
      imageUrl: null,
    };
  });

  for (const p of randomPlaces) {
    await prisma.place.create({ data: p });
  }

  console.log(`Successfully generated 500 randomized places.`);
